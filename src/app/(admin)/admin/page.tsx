...
import { connectToDatabase, User, HostingApplication, ManagedDatabase, SupportTicket, AuditLog } from '@/lib/db';
...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-normal font-serif text-[var(--text-primary)] tracking-tight">Admin Control Center</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Link href="/admin/customers" className="card hover:border-[var(--primary)] transition-colors">
          <span className="text-xs text-[var(--muted)] uppercase tracking-wider font-medium">Total Customers</span>
          <p className="text-3xl font-normal font-serif text-[var(--text-primary)] mt-2">{totalUsers}</p>
          <span className="text-xs text-[var(--text-secondary)] mt-1 block">Registered accounts</span>
        </Link>

        <Link href="/admin/applications" className="card hover:border-[var(--primary)] transition-colors">
          <span className="text-xs text-[var(--muted)] uppercase tracking-wider font-medium">Pending Applications</span>
          <p className={`text-3xl font-normal font-serif mt-2 ${pendingApps > 0 ? 'text-[var(--warning)]' : 'text-[var(--text-primary)]'}`}>
            {pendingApps}
          </p>
          <span className="text-xs text-[var(--text-secondary)] mt-1 block">Requires manual review</span>
        </Link>

        <Link href="/admin/provisioning" className="card hover:border-[var(--primary)] transition-colors">
          <span className="text-xs text-[var(--muted)] uppercase tracking-wider font-medium">Active Databases</span>
          <p className="text-3xl font-normal font-serif text-[var(--success)] mt-2">{activeDbs}</p>
          <span className="text-xs text-[var(--text-secondary)] mt-1 block">Managed deployments</span>
        </Link>

        <Link href="/admin/support" className="card hover:border-[var(--primary)] transition-colors">
          <span className="text-xs text-[var(--muted)] uppercase tracking-wider font-medium">Open Tickets</span>
          <p className={`text-3xl font-normal font-serif mt-2 ${openTickets > 0 ? 'text-[var(--warning)]' : 'text-[var(--text-primary)]'}`}>
            {openTickets}
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="card space-y-2">
          <h3 className="text-base font-normal font-serif text-[var(--text-primary)]">Onboarding Pipeline</h3>
        ...
        <div className="card space-y-2">
          <h3 className="text-base font-normal font-serif text-[var(--text-primary)]">Provisioning</h3>
        ...
        <div className="card space-y-2">
          <h3 className="text-base font-normal font-serif text-[var(--text-primary)]">Billing &amp; Payments</h3>
